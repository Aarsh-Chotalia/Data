...
C24_idx=[];
C24modidx=[];
t_range=(transient+1/Fs+max_delay+window/2)*Fs+1:5000:time_range(end)-window*Fs/2;
for t=t_range
    t/1000
    [mx,idx]=maxcor(x1(t-window*Fs/2:t+window*Fs/2),x3(t-window*Fs/2:t+window*Fs/2),max_delay*Fs);
    C13=[C13,mx];
    C13_idx=[C13_idx,idx];
...
%     C13modidx_sin=[C13modidx_sin,modidx];
    [mx,idx]=maxcor(x2(t-window*Fs/2:t+window*Fs/2),x4(t-window*Fs/2:t+window*Fs/2),max_delay*Fs);
    C24=[C24,mx];
    C24_idx=[C24_idx,idx];
%     modidx=mod(idx/1000,0.1);
%     modidx_cos=cos(2*pi*modidx/0.1);
...
legend(["R13="+num2str(R13) "R24="+num2str(R24)])
xlabel("Time (s)")
ylabel("cos\theta_\tau")
ylim([-1 1])
set(gca,'FontSize',FontSize)
...
saveas(gcf,"S"+num2str(Subject_id)+"_corr_window",'png')
fname="S"+ num2str(Subject_id)+"corr__window_data"
save(fname)
